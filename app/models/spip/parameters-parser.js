      ...
      if (query.boucle.jointures && query.boucle.jointures.id_mot) {
        if (Array.isArray(query.raw.balises)) {
          ...
          if (mots > -1) {
            ...
            query.motsAssocies = ", CONCAT('[',GROUP_CONCAT(DISTINCT CONCAT('{\"id_mot\":',spip_mots.id_mot,',\"titre\":\"',spip_mots.titre,'\",','\"id_groupe\":',spip_mots.id_groupe,'}')),']') AS mots ";
            ...
            for (let i = 0, l = query.raw.id.length; i < l; i++) {
              if (Array.isArray(query.raw.liens[id_join])) { //plusieurs objet à associer
                for (let j = 0, len = query.raw.liens[id_join].length; j < len; j++)
                  query.liens.push(_makeInsertSetForLiens(query, {
                    id_objet: query.raw.id[i],
                    id_join_name: id_join,
                    id_join_value: query.raw.liens[id_join][j]
                  }));

              } else query.liens.push(_makeInsertSetForLiens(query, {
                id_objet: query.raw.id[i],
                id_join_name: id_join,
                id_join_value: query.raw.liens[id_join]
              }));
...
function _makeInsertSetForLiens(query, {
  id_objet,
  id_join_name,
  id_join_value
}) {
  let insert_set = {
    objet: query.boucle.nom,
    id_objet: id_objet
  };
